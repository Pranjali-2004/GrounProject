  ...
  Widget build(BuildContext context) {
    return Scaffold(extendBodyBehindAppBar: true,
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor:
        Colors.blue, //Color.fromARGB(255, 32, 146, 36),
        onPressed: (){
...
// List view builder function
  ListView buildListView() {int i=1;
    return ListView.builder(itemBuilder: (context,index) {
            return (i%2) ==0 ?
            Card(child: ListTile(title: Text(details[i].toString(),style: const TextStyle(fontSize: 20),),
              subtitle: Card(child: ListTile(title: Text(details[i+1].toString(),style: const TextStyle(fontSize: 20), ),),),),) :

                  Card(child: ListTile(title: Text(details[index].toString(),style: const TextStyle(fontSize: 20),), ),);
            },
